...
def get_user(username):
    ...
    return jsonify({
        ...
        'email': user['email'],
        'phone': user.get('phone', '') if hasattr(user, 'get') and user.get('phone') else (user['phone'] if 'phone' in user.keys() else ''),
        'pin': user['pin'],  # Include PIN pentru verificare
        'birthDate': user['birth_date'],
        'affirmation': user['affirmation'],
        'avatar': user['avatar'],
        'role': user.get('role', 'user') if hasattr(user, 'get') else (user['role'] if 'role' in user.keys() else 'user'),
